...
# %% [markdown]
# # GeoVision-CLIP — Notebook 1: muestreo estratificado para Situacion 2
#
# Dataset montado: `juanjoseorozcolopez/geovision-fuentes`
# Genera tiles 64x64x13 etiquetados por percentiles de S5P + NDVI/NDBI sobre Cali.
#
# 1. Imports + constantes
# 2. Apertura de paneles, parseo de tiempos, percentiles
# 3. Geometria de tiles, NDVI/NDBI, SCL, texto descriptivo
# 4. Reglas de clase + evaluacion de candidato
# 5. Pipeline target-driven `run(n_tiles, dry_run)`
# 6. **TEST** (25 tiles, dry-run)
# 7. **FULL** (5000 tiles, escribe en /kaggle/working/) — descomentar tras TEST OK
...
# %% [markdown]
# ## Apertura de paneles + parseo de tiempos + percentiles S5P
...
def parse_kaggle_time(s):
    """Parsea formatos system_index de GEE y variantes ERA5/ISO."""
    s = str(s)
    if "_" in s:
        s = s.split("_")[0]
...
_TIMES_CACHE = {}
def times_parseados(panel, name):
...
# %% [markdown]
# ## Geometria de tiles, NDVI/NDBI, SCL, texto descriptivo
...
def proponer_un_candidato(s2, rng):
    half = TILE_PX // 2
...
def valor_s5p(panel, lat, lon, banda, t_idx):
...
def scl_valido(tile, bands):
    scl = tile[bands.index("SCL")]
    return float(np.isin(scl, [4, 5, 6, 7]).mean())


def generar_texto(clase, no2, so2, o3, ndvi):
    return {
...
# %% [markdown]
# ## Reglas de clase + evaluacion de candidato

# %%
def cand_cumple(cand, clase, perc):
...
def evaluar_candidato(cand, s2, no2, so2, o3, bands):
...
# %%
def run(n_tiles, dry_run, max_intentos_factor=80):
...
# %% [markdown]
# ## TEST — 25 tiles, dry-run

# %%
aceptados, perc, stats = run(n_tiles=25, dry_run=True)

# %% [markdown]
# ## FULL — 5000 tiles, escribe `tiles_train.npz` + `tiles_meta.parquet`
#
# Descomentar despues de validar que el TEST corre sin errores y produce ~5 tiles por clase.
